...
// dencrpt   
    public BigInteger dencrypt(BigInteger c, BigInteger n, BigInteger d) {
        BigInteger clear = c.modPow(d, n);
        return clear;
    }

// encrypt  
    public String encryptSignature(String s, String d1, String n1) {
        ...
        try {
            ...
            BigInteger b1 = strToBig(dt);

            BigInteger encrypt = b1.modPow(d, n);
            String resultAfter = DatatypeConverter.printHexBinary(encrypt.toByteArray());
            return resultAfter;
        } catch (NoSuchAlgorithmException ex) {
            Logger.getLogger(RSAExample.class.getName()).log(Level.SEVERE, null, ex);
            return null;
        }
    }

    public BigInteger strToBig(byte[] dt) {
        byte[] dt1 = {0};
